...
//change mongoose to use q promise library instead of built in / deprecated promises
...
var NdPrincipal = require('./auth/NdPrincipal.js');
var CoreTemplate = require('./CoreTemplate.js');
...
var Q = require('q');
	(function(mongoose, underscore, ndconfig, hx$, moment, crypto, jwt, uuid, ndschemas, ndfilters)
	   ...
	   root.config = ndconfig;
		...
		root.GenerateDependencies = function(model, dependencies)
		{
			...
			if(ndschemas.Schemas[model])
			{
				for(var member in ndschemas.Schemas[model]) {
					...
					if(name != undefined && !hx$.singleOrDefault(dependencies, (element) => {return element[1] === name})) {
		...
		/*END HELPERS for MONGOOSE updates and denormalization*/
	       ...
	       root.GenerateRoute = function(args)
					   ...
					   return hx$.ApplyArgs(arguments, (arg, accumulator) => {return '/' + arg;});
				       };
       ...
       root.authentication = {
		   ...
		   createKey: function(authHeaders, userId, response, relativePath)
		   {
			   ...
			   authenticated.then(function(principle)
			   {
				   ...
				   //verify that the userId is of type string
				   if(typeof(userId) !== 'string')
				   {
					   deferred.reject(new Error('Cannot create a key with a non string userId'));
				   }
		   ...
		   authenticateApiKey : function(key, authCfg, deferred, response)
		   {
			   ...
			   model.findOne({content : key}, function(err, element)
			   {
				   ...
				   else
				   {
					   ...
					   var decodeResult = jwt.verify(element.content, privateKey , {algorithm: 'HS256'}, function(err, decoded)
					   {
						   if (err)
						   {
							   deferred.reject(new Error('Authorization failure. Invalid Api Key'));
						   ...
						   else
						   {
							   ...
							   root.connection.connect(function() {
								   hx$.log('matching user to key with userId: ' + userId);
			   ...
			   hx$.log('authenticating password');
				   ...
				   if(user === authCfg.master.uname && authCfg.master.masterEnabled)
				   {
					   ...
					   if(digest === authCfg.master.password)
					   {
						   ...
						   var principle = new NdPrincipal({name : user, role: {}, master : 1});
					   ...
					   root.connection.connect(function() {
						   ...
						   model.findOne({'name': user}, function (err, usr) {
							   if (!root.authentication.checkAuthError(err, deferred)) {
								   ...
								   var newHash = root.authentication.hash(password, authCfg.master.hash);
								   if (usr.password === newHash) {
	   ...
	   //helper function for joining models once they have been returned.
		...
		//resolve theme method

		root.ResolveTheme = function(idValue, principal)
		{
			//to resolve a theme, you must have permissions to access it.
            principal.ValidateForInterface('theme', 'get');
			root.connection.connect(function()
			{
				var deferred = Q.defer();
				var result = root.interfaces.GetId('theme', idValue, null, null, principal);
				//first, get the theme by id
				result.then(function(themeContent)
				{
					var coreTemplate = new CoreTemplate({text: themeContent});
					//next, compile the theme using CoreTemplates
					var rendered = coreTemplate.renderTemplate();
	   ...
	   root.interfaces = {
		...
		if (typeof define === 'function' && define.amd) {
			define('ndcore', ['mongoose', 'underscore', 'ndconfig', 'hx$', 'moment', 'crypto', 'ndschemas', 'NdPrincipal'], function() {
				return root;
			});
		}
	       return root;
