       ...
       root.authentication = {
		...
		root.authentication.Principle.prototype.ValidateForInterface = function(resource, action, resourceValue)
		{
			...
			else
			{
				...
				var passSiteScopedAccess = (!self.role.siteScoped) || ((resource === 'site' && resourceValue.id === self.role.siteScopedId)
					|| (resourceValue.siteId === self.role.siteScopedId)
					|| (!resourceValue.siteId));
				var passUserScopedAccess = (!self.role.userScoped) ||  ((resource === 'user' && resourceValue.id === self.role.userScopedId)
					|| (resourceValue.userId === self.role.userScopedId)
