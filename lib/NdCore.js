...
var Q = require('q');
	(function(mongoose, underscore, ndconfig, hx$, moment, crypto, jwt, uuid, ndschemas)
       {
	   var root = {};
	   ...
	   root.config = ndconfig;
		...
		root.AssignSchemaPropertiesFromUpdate = (function(element, resourceValue)
		{
			...
			{
				if(element._doc.hasOwnProperty(prop) && prop !== 'id' && prop !== '_id')
				{
		...
		root.authentication.Principle.prototype.ValidateForInterface = function(resource, action)
		{
		   ...
		   root.filters = {
			   'user' : [{name : 'password', func : 'hashpw', type : 'field'},
				   {name : 'gender', func : 'validateGender', type : 'field'}],
			   'image' : [
				   {name : 'data', func : 'transformImage', type : 'field'}
			   ],
	   ...
	   root.interfaces = {
		...
		if (typeof define === 'function' && define.amd) {
	       ...
	       return root;
	   })(mongoose, underscore, ndconfig, hx$, moment, crypto, jwt, uuid, ndschemas);
