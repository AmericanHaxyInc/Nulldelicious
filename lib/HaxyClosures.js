    ...
    (function () {
        ...
        //logging...
        root.LoggingEnabled = 1;
        //logging mutator/accessor
        root.ConsoleLoggingEnabled = (function (arg) {
            if (arg === undefined) {
                return root.LoggingEnabled;
            }
            else {
                root.LoggingEnabled = arg;
                return arg;
            }
        });
        //log gate
        root.Log = (function (text) {
            if (root.ConsoleLoggingEnabled()) {
                console.log(text);
            }
        });
        //aliasing
        ...
        root.CopyProps = (function(model1, model2)
        {
            for(var element2 in model2)
            {
                for(var element1 in model1)
                {
                    if(element1 === element2 && model1.hasOwnProperty(element1))
                    {
