...
var fixturePromises = [siteScopedFixtures.promise];
...
//user 3 data
var role3Fixture = {id: role3Id, name: "FullControlUser3Site3", access: access, siteId: site2Id, siteScoped: true, userScoped: true, userId : user3Id};
        ...
        authenticated.then(function(principal)
        {
            ...
            }).then(function(user)
            {
                ...
                key.then(function(key)
                {
                    ...
                    var keyAuthenticationHeaders = {"x-nd-apikey" : key.content}
                    keyAuthentication.then(function(principal2)
                    {
                        ...
                        }).then(function(user2)
                        {
                            ...
                            return ndcore.authentication.createKey(keyAuthenticationHeaders, user2Id, {});
                        }).then(function(key2value)
                        {
                            key2 = key2value.content;
                        ...
                        }).then(function(user3)
                        {
                            ...
                            return ndcore.authentication.createKey(keyAuthenticationHeaders, user3Id, {});
                        }).then(function(key3value)
                        {
                            key3 = key3value.content;
                            hx$.log('Key 3 created. Testing site and user scoped assertions');
                            siteScopedFixtures.resolve(true);
                        }).catch(function(err)
                        {
                            hx$.log('Step 1 failure');
                            hx$.log(err);
                        });
...
var key2Tests = Q.defer();
var key2Promises = [key2Tests.promise];
var key3Prerequisites = Q.all(fixturePromises.concat(key2Promises));
var postFixtures = Q.all(fixturePromises);
postFixtures.then(function(result)
{
    hx$.log('starting site scoped and user scoped tests');
    hx$.log('----- SECOND SECTION TESTS SITE SCOPED TOKENS----- ');
    ...
    ndcore.authentication.authenticate({"x-nd-apikey" : key2}, 'post', 'set',  {})
        .then(function(principal2)
        {
            ...
            }).then(function(site2)
            {
                ...
                assert.fail(0, 0, 'improper permissions');
            }).catch(function(error)
            {
                if(error.message && error.message !== 'improper permissions') {
                    hx$.log('site failed successfully');
                    //we expect failure on our final step
                    assert.ok(error);
                }
                //now go to our user scoped tests...
                key2Tests.resolve();
            });
        });
}).catch(function(err)
...
key3Prerequisites.then(function(result)
{
    hx$.log('----- THIRD SECTION USER SCOPED TOKENS----- ');

    var currentPrincipal = {};
    ndcore.authentication.authenticate({"x-nd-apikey" : key3}, 'post', 'set', {}).then(function(principal3)
    {
